...
export default function QuickOrderForm({ product, onSuccess }: QuickOrderFormProps) {
  ...
  return (
    <div>
      <div className="flex items-center gap-2 mb-4">
        ...
        <h3 className="font-display text-lg font-bold text-secondary-900">Passer commande</h3>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <div className="flex items-center gap-2">
              <button type="button" onClick={() => setQuantity((q) => Math.max(1, q - 1))} className="w-10 h-10 rounded-lg border border-secondary-200 flex items-center justify-center text-secondary-600 hover:bg-secondary-50">−</button>
              <input type="number" min={1} value={quantity} onChange={(e) => setQuantity(Math.max(1, parseInt(e.target.value) || 1))} className="input text-center w-20" />
              <button type="button" onClick={() => setQuantity((q) => q + 1)} className="w-10 h-10 rounded-lg border border-secondary-200 flex items-center justify-center text-secondary-600 hover:bg-secondary-50">+</button>
            </div>
          </div>
        ...
        <button type="submit" disabled={submitting} className="btn-primary w-full !py-3">
          {submitting ? <><Loader2 className="w-4 h-4 animate-spin" /> Envoi en cours...</> : 'Confirmer la commande'}
        </button>
      </form>
